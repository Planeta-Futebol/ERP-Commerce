...
//        }
    ...
    public function processDataWhenEditOrder(){
        ...
        $currentCouponCode = '';
        $couponCodeBySession = $this->getCheckoutSession()->getAffiliateCouponCode();
        ...
        if($orderId){
            ...
            $transactionAffiliate = $this->_getAffiliateTransactionModel()
                ->load($orderIncrementId, 'order_number');
            ...
            // check lifetime
            $customer = $this->_backendQuoteSession->getCustomer();
            $customerId = $this->_backendQuoteSession->getCustomer()->getId();
            ...
            if($accountInfo){
                ...
                $defaultDiscount = false;
                $programId = '';
                $programInfo = new \Magento\Framework\DataObject(array(
                    'program_id'	=> $programId,
                ));
                $this->_eventManager->dispatch('affiliateplus_program_transaction_data',array(
                ...
                $accountInfo = $this->_accountFactory->create()->load($affId);
                if($accountInfo->getStatus() == 2 && $couponCodeBySession == '')
                    return $result;
            }
            $currentAff = $this->_accountFactory->create()->load($affId);
            if(!$convertLifetime){
            ...
            if($this->isAdmin()){
                ...
                if($this->_backendQuoteSession->hasCustomerId()){
                    $customerId = $this->_backendQuoteSession->getCustomerId();
                    $accountInfo = $this->checkLifeTimeForOrderBackend($customerId);
                    if($accountInfo){
                        $defaultDiscount = true;
                    }else{
        ...
        if($couponCodeBySession && $this->isAdmin()){
            $programAndAccountData = new \Magento\Framework\DataObject(array(
                'program_id'	=> '',
                'account_info' =>  '',
                'program_name' => ''
            ));
            $this->_eventManager->dispatch('affiliateplus_coupon_get_program_data_and_account',array(
